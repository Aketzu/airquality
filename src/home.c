...
void i2c_task(void *par) {
  ...
  for (;;) {
    ...
    // GMx02B, Gas sensors
    sensor_send(gm, "\x01", 1, 0, &no2, sizeof(no2));
    sensor_send(gm, "\x03", 1, 0, &alc, sizeof(alc));
    sensor_send(gm, "\x05", 1, 0, &voc, sizeof(voc));
    sensor_send(gm, "\x07", 1, 0, &co, sizeof(co));
...
void display_home_tab(lv_obj_t *tv) {
  ...
  lv_obj_align(tab_title_label, home_tab, LV_ALIGN_IN_TOP_MID, 0, 5);
  ...
  size_t arrow_buf_len = snprintf (NULL, 0,
      "#ff9900 %1$s       %1$s       %1$s#       #232f3e SWIPE #      #ff9900
  %1$s       %1$s       %1$s#       #232f3e SWIPE #", LV_SYMBOL_LEFT); char
  arrow_buf[++arrow_buf_len]; snprintf (arrow_buf, arrow_buf_len,
      "#ff9900 %1$s       %1$s       %1$s#       #232f3e SWIPE #      #ff9900
  %1$s       %1$s       %1$s#       #232f3e SWIPE #", LV_SYMBOL_LEFT);
  ...
  */
  ...
  xTaskCreatePinnedToCore(i2c_task, "I2CTask", 4096, (void *)body_label, 1,
                          NULL, 1);
